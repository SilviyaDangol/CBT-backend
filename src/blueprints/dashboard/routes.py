...
def teacher_stats_by_admin():
    ...
    if is_admin:
        ...
        total_classes = Classroom.query.filter_by(teacher_id=user_id).count()
        total_sessions = Session.query.join(Classroom).filter(Classroom.teacher_id == user_id).count()

    # --- 2. Behavior Distribution (All Time) ---
    behavior_counts = []
    for behavior in BEHAVIOR_TYPES:
        query = Behaviour.query.filter_by(behaviour=behavior)
        if not is_admin:
            query = query.join(Session).join(Classroom).filter(Classroom.teacher_id == user_id)
        count = query.count()
        behavior_counts.append({"behavior": behavior, "count": count})

    # --- 3. Class-Wise Behavior Breakdown ---
    class_stats = []
    classes = Classroom.query.all() if is_admin else Classroom.query.filter_by(teacher_id=user_id).all()

    for class_obj in classes:
        session_count = len(class_obj.sessions)

        behaviors = []
        for behavior in BEHAVIOR_TYPES:
            count = Behaviour.query.join(Session).filter(
